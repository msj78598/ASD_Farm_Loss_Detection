...
def save_results_html(rows: List[List], colors: dict, detected_dir: str) -> bytes:
    ...
    for r in rows:
        # هنا عدّلنا ترتيب الأعمدة لاحقًا لذلك انتبه إذا غيّرت res_df
        meter_id, priority, risk, distance, area, consumption, breaker, office, lat, lon, *_ = r
...
def download_image(lat: float, lon: float, meter_id: str,
                   acq_date: Optional[str] = None,   # NEW
                   timeout: int = 30):
    """
...
if uploaded:
    ...
    # ========== إدخال تواريخ صور شهر الاستهلاك ==========
    dates_str = st.sidebar.text_input(
        "تواريخ صور شهر الاستهلاك (YYYY-MM-DD مفصولة بفاصلة)",
        value=""
    )
    if dates_str.strip():
        analysis_dates = [d.strip() for d in dates_str.split(",") if d.strip()]
    else:
        analysis_dates = []   # لو فاضي → يستخدم السلوك القديم (أحدث صورة)
    ...
    if st.sidebar.button("📥 تنزيل/عرض الصور"):
        ...
        for i, (_, row) in enumerate(df.iterrows(), 1):
            ...
            # هنا نستخدم السلوك التقليدي (أحدث صورة)
...
<div style="border:1px solid #ddd;border-radius:8px;padding:6px;margin:6px;text-align:center">
  ...
  <small>عداد {meter}<br>Lat {lat:.6f}, Lon {lon:.6f}</small>
    ...
    if st.sidebar.button("🚀 بدء التحليل"):
        ...
        progress = st.sidebar.progress(0)
        results = []
        ts_rows = []   # NEW: سلسلة زمنية لكل (عداد + تاريخ)
        cols, col_i = st.columns(3), 0
        t0 = time.time()
        n = len(df)
        ...
        for i, (_, row) in enumerate(df.iterrows(), 1):
            try:
                ...
                # لو ما فيه تواريخ محددة → نستخدم None (أحدث صورة فقط)
                dates_for_meter = analysis_dates if analysis_dates else [None]
                ...
                for d in dates_for_meter:
                    ...
                    if det is None:
                        # نحفظ Row في السلسلة الزمنية مع green_ratio=0 لو حاب
                        ts_rows.append([
                            meter, d, 0.0, 0, cons, br, off, lat, lon
                        ])
                        continue

                    dets_for_meter.append((d, det))
                    # نخزن في السلسلة الزمنية
                    ts_rows.append([
                ...
                if not dets_for_meter:
                    # لا يوجد كشف صالح لهذا العداد في أي تاريخ
                    progress.progress(i / n)
                    continue

                # ملخص شهري للحقل: نأخذ أكبر مساحة / أعلى خضرة
                best_d, best_det = max(dets_for_meter, key=lambda x: x[1].green_ratio)

                # متوسط خضرة الشهر لهذا الحقل
                green_vals = [d[1].green_ratio for d in dets_for_meter]
                ...
                # نعرض صورة أفضل تاريخ (أعلى خضرة)
...
<div style="border:4px solid {colors.get(pr,'#ccc')};padding:10px;border-radius:12px;margin:6px;text-align:center;">
  ...
  خضرة (أفضل صورة):{best_det.green_ratio*100:.0f}% | متوسط خضرة الشهر:{green_mean*100:.0f}%<br>
  ...
  <a href="https://maps.google.com?q={lat},{lon}">📍 الموقع</a>
        ...
        if results:
            res_df = pd.DataFrame(results, columns=[
                ...
                "green_ratio_mean_month","num_valid_dates"
            ])
        ...
        if ts_rows:
            ...
            st.sidebar.download_button(
                "📥 السلسلة الزمنية (كل عداد + تاريخ) (Excel)",
