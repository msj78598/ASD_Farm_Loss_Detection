...
def save_results_html(rows: List[List], colors: dict, detected_dir: str) -> bytes:
    ...
    for r in rows:
        # [meter, pr, score, edge_d, center_d, area, cons, br, off, lat, lon]
...
def detect_field_progressive(img_path, lat, lon, meter_id, model_yolo, calibration_factor, min_conf_accept, min_area_m2, detected_dir, r_start=50, r_step=10, r_max=200):
    ...
    draw.text((int(box[0]) + 4, int(box[1]) + 4), f"R<= {chosen_R}m | Edge:{edge_dist:.1f}m | Center:{center_dist:.1f}m | Green {green_ratio*100:.0f}%", fill="white")
...
def get_cdse_token():
    ...
    if not cid or not csec: raise RuntimeError("CDSE_CLIENT_ID / CDSE_CLIENT_SECRET غير موجودة")
    data = {"grant_type":"client_credentials","client_id":cid,"client_secret":csec}
    r = requests.post(TOKEN_URL, data=data, timeout=20)
    if r.status_code != 200: raise RuntimeError(f"CDSE token error {r.status_code}")
    js = r.json()
...
if uploaded:
    ...
    if st.sidebar.button("🚀 بدء التحليل"):
        ...
        results, cols, col_i, n = [], st.columns(3), 0, len(df)
        t0 = time.time()
...
<div style="border:4px solid {colors.get(pr,'#ccc')};padding:10px;border-radius:12px;margin:6px;text-align:center;">
  ...
  <a href="https://maps.google.com?q={lat},{lon}">📍 الموقع</a>
        ...
        if results:
            ...
            st.sidebar.download_button("📥 تقرير HTML", data=save_results_html(results, colors, cfg.detected_dir), file_name="report.html", mime="text/html")
