...
st.set_page_config(page_title="نظام الرصد المطور 2026", layout="wide")
...
# ======================= 4. منطق الخضرة "القوي" (الذي تفضله) =======================
...
def detect_field_progressive(img_path, lat, lon, m_id, model_yolo):
    ...
    for box in res.boxes:
        ...
        candidates.append({'edge': max(dist - radius_m, 0.0), 'center': dist, 'box': xyxy, 'area': int(area), 'green': g_ratio})
...
if uploaded:
    ...
    if st.sidebar.button("🚀 بدء التحليل المطور"):
        ...
        for i, (_, row) in enumerate(df.iterrows()):
            ...
            try:
                ...
                with grid[i % 3]:
                    st.image(det['path'], caption=f"عداد {m_id} - {pr}")
        ...
        if results:
            ...
            st.download_button("📥 تحميل النتائج Excel", data=res_df.to_csv(index=False).encode('utf-8-sig'), file_name="results.csv")
