...
def detect_field_progressive(
    ...
    ✅ Progressive search by EDGE distance:
    - R = 50, 60, 70, ..., 200
    - At first R where any candidate satisfies edge_distance <= R:
        choose min(edge_distance, center_distance) and STOP.
    """
        ...
        for i, (_, row) in enumerate(df.iterrows(), 1):
            try:
                ...
                det = detect_field_progressive(
                    ...
                    r_start=cfg.r_start_m, r_step=cfg.r_step_m, r_max=cfg.r_max_m
                )
  ...
  <a href="https://maps.google.com?q={lat},{lon}">📍 الموقع</a>
        ...
        if results:
            res_df = pd.DataFrame(results, columns=[
                "Subscription","priority","risk_score","edge_distance_m","center_distance_m","area_m2",
